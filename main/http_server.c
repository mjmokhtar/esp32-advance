 ...
 * @return ESP_OK
 */
static esp_err_t http_server_get_md02_sensor_readings_json_handler(httpd_req_t *req)
{
	ESP_LOGI(TAG, "/md02Sensor.json requested");

	char md02SensorJSON[100];

	sprintf(md02SensorJSON, "{\"temp\":\"%.1f\",\"humidity\":\"%.1f\"}", XY_MD02_getTemperature(), XY_MD02_getHumidity());

	httpd_resp_set_type(req, "application/json");
	httpd_resp_send(req, md02SensorJSON, strlen(md02SensorJSON));
...
static httpd_handle_t http_server_configure(void)
{
	...
	if (httpd_start(&http_server_handle, &config) ==  ESP_OK)
	{
		...
		// register md02Sensor.json handler
		httpd_uri_t md02_sensor_json = {
				.uri ="/md02Sensor.json",
				.method = HTTP_GET,
				.handler = http_server_get_md02_sensor_readings_json_handler,
				.user_ctx = NULL
		};
		httpd_register_uri_handler(http_server_handle, &md02_sensor_json);
